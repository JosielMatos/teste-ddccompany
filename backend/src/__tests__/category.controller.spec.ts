import { HttpController } from '../modules/category/http-controller'
import { Service } from '../modules/category/service'
import { EntityResponse } from '../modules/category/entities/entity'
...
describe('CategoryController', () => {
  ...
  it('should call service.execute with parsed query params', async () => {
    ...
    const query = {
      skip: '10',
      take: '5',
      andWhere: JSON.stringify([{ field: 'name', value: 'category1' }]),
      orWhere: JSON.stringify([{ field: 'type', value: 'main' }]),
      andWhereNot: JSON.stringify([{ field: 'active', value: false }]),
      orWhereNot: JSON.stringify([{ field: 'parent', value: 'none' }]),
      orderBy: JSON.stringify([{ field: 'id', direction: 'DESC' }]),
    }
    const expectedData = {
      skip: 10,
      take: 5,
      andWhere: [{ field: 'name', value: 'category1' }],
      orWhere: [{ field: 'type', value: 'main' }],
      andWhereNot: [{ field: 'active', value: false }],
      orWhereNot: [{ field: 'parent', value: 'none' }],
  ...
  it('should call service.execute on create', async () => {
    const req = { tokenData: { userId: 1 } }
    const data = { name: 'Category Test', type: 'main', active: true }
    const fakeResponse: EntityResponse = {
      data: { items: [{ id: 1, name: 'Category Test', createdAt: new Date(), updatedAt: new Date() }], count: 1 },
      error: undefined,
    }
    service.execute.mockResolvedValue(fakeResponse)
  ...
  it('should call service.execute on getById with valid id', async () => {
    ...
    const id = '42'
    const fakeResponse: EntityResponse = {
      data: { items: [{ id: 42, name: 'Category 42', createdAt: new Date(), updatedAt: new Date() }], count: 1 },
      error: undefined,
    }
    service.execute.mockResolvedValue(fakeResponse)
  ...
  it('should call service.execute on update with valid id', async () => {
    const req = { tokenData: { userId: 1 } }
    const id = '10'
    const data = { name: 'Updated Category', type: 'main', active: false }
    const fakeResponse: EntityResponse = {
      data: { items: [{ id: 10, name: 'Updated Category', createdAt: new Date(), updatedAt: new Date() }], count: 1 },
      error: undefined,
    }
    service.execute.mockResolvedValue(fakeResponse)
