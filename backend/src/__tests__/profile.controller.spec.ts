import { HttpController } from '../modules/profile/http-controller'
import { Service } from '../modules/profile/service'
import { EntityResponse } from '../modules/profile/entities/entity'
...
describe('ProfileController', () => {
  ...
  it('should call service.execute on create', async () => {
    const req = { tokenData: { userId: 1 } }
    const data = { name: 'Test Profile', email: 'profile@example.com', user: {} as any }
    const fakeResponse: EntityResponse = {
      data: { items: [{ id: 1, userId: 1, createdAt: new Date(), updatedAt: new Date() }], count: 1 },
      error: undefined,
    }
    service.execute.mockResolvedValue(fakeResponse)
  ...
  it('should call service.execute on getById with valid id', async () => {
    ...
    const fakeResponse: EntityResponse = {
      data: {
        items: [{ id: 42, userId: 1, createdAt: new Date(), updatedAt: new Date() }],
        count: 1,
      },
      error: undefined,
    }
  ...
  it('should call service.execute on update with valid id', async () => {
    const req = { tokenData: { userId: 1 } }
    const id = '10'
    const data = { name: 'Updated Profile', email: 'profile@example.com', user: {} as any }
    const fakeResponse: EntityResponse = {
      data: {
        items: [{ id: 10, userId: 1, createdAt: new Date(), updatedAt: new Date() }],
        count: 1,
      },
      error: undefined,
    }
