...
export class Service {
  ...
  create: MethodType<ExecutionDTOType<CreateDTO, 'create'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-criar-category'] }))
            .permitted
        ) {
          return data
        }
  ...
  get: MethodType<ExecutionDTOType<GetDTO, 'get'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-ler-category'] })).permitted
        ) {
          return data
        }
        data.error = { error: { forbidden: 'Sem permissão para o recurso' } }
  ...
  update: MethodType<ExecutionDTOType<UpdateDTO, 'update'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-atualizar-category'] }))
            .permitted
        ) {
          return data
        }
      ...
      checkExists: async (data) => {
        ...
        if (!result) {
          data.error = { error: { notFound: 'Category não encontrada' } }
        }
        return data
      },
    },
  ...
  delete: MethodType<ExecutionDTOType<DeleteDTO, 'delete'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-deletar-category'] }))
            .permitted
        ) {
          return data
        }
    ...
    execution: async (data: ExecutionDTOType<DeleteDTO, 'delete'>) => {
      ...
      await this.prisma.postCategory.deleteMany({
        where: { categoryId: data.datap.id },
      })
