...
export class Service {
  ...
  create: MethodType<ExecutionDTOType<CreateDTO, 'create'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-criar-large-table'] }))
            .permitted
        ) {
          return data
        }
  ...
  get: MethodType<ExecutionDTOType<GetDTO, 'get'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-ler-large-table'] }))
            .permitted
        ) {
          return data
        }
  ...
  update: MethodType<ExecutionDTOType<UpdateDTO, 'update'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-atualizar-large-table'] }))
            .permitted
        ) {
          return data
        }
      ...
      checkExists: async (data) => {
        ...
        if (!result) {
          data.error = { error: { notFound: 'LargeTable não encontrado' } }
        }
        return data
      },
    },
  ...
  delete: MethodType<ExecutionDTOType<DeleteDTO, 'delete'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-deletar-large-table'] }))
            .permitted
        ) {
          return data
        }
