...
export class Service {
  ...
  create: MethodType<ExecutionDTOType<CreateDTO, 'create'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-criar-profile'] })).permitted
        ) {
          return data
        }
      ...
      checkUserExists: async (data) => {
        if (data.datap.user?.connect?.id) {
          const userExists = await this.prisma.user.findUnique({
            where: { id: data.datap.user.connect.id },
          })
          if (!userExists) {
            data.error = { error: { notFound: 'Usuário não encontrado' } }
          }
        }
        return data
      },
      uniqueUserId: async (data) => {
        if (data.datap.user?.connect?.id) {
          const result = await this.prisma[moduleMetadata.tb].findFirst({
            where: {
              userId: data.datap.user.connect.id,
            },
          })
          if (result) {
            data.error = {
  ...
  get: MethodType<ExecutionDTOType<GetDTO, 'get'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-ler-profile'] })).permitted
        ) {
          return data
        }
        data.error = { error: { forbidden: 'Sem permissão para o recurso' } }
  ...
  update: MethodType<ExecutionDTOType<UpdateDTO, 'update'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-atualizar-profile'] }))
            .permitted
        ) {
          return data
        }
      ...
      checkExists: async (data) => {
        ...
        if (!result) {
          data.error = { error: { notFound: 'Profile não encontrado' } }
        }
        return data
      },
    },
  ...
  delete: MethodType<ExecutionDTOType<DeleteDTO, 'delete'>, EntityResponse> = {
    rules: {
      perm: async (data) => {
        if (
          (await checkPermission({ prisma: this.prisma, token: data.tokenData, data: ['api-deletar-profile'] }))
            .permitted
        ) {
          return data
        }
